...
## 7. Interactive Cloud Architecture & Infrastructure as Code Patterns

To illustrate modern enterprise cloud integration, consider this serverless telemetry and edge delivery architecture:
...
The underlying infrastructure is codified using declarative Terraform:
...
resource "aws_apigatewayv2_api" "http_api" {
  name          = "portfolio-telemetry-gateway"
  protocol_type = "HTTP"
  cors_configuration {
    allow_origins = ["https://www.oliver-slater.co.uk"]
    allow_methods = ["POST", "OPTIONS"]
  }
}
```

### Containerized Task Definition (`dockerfile`)

For build workers and offline synthetic tests:
...
### Automation & Deployment (`bash` & `python`)

```bash
#!/usr/bin/env bash
set -euo pipefail

echo "Executing automated deployment and cache invalidation..."
aws s3 sync ./dist s3://oliver-slater-edge/ --delete
...
def lambda_handler(event, context):
    print("Received portfolio telemetry ping")
    return {
        "statusCode": 200,
        "headers": {"Content-Type": "application/json"},
        "body": json.dumps({"status": "healthy", "service": "portfolio-edge"}),
    }
```
